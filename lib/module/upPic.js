...
module.exports = function (req, typeImg, folderName, cb) {
    const form = new formidable.IncomingForm();
    const uploadDir = path.resolve(__dirname, "../upload/" + folderName);
    form.encoding = "utf-8";
    form.uploadDir = uploadDir;
    form.keepExtensions = true;// 保留扩展名
    form.parse(req, (err, params, file) => {
        const picInfo = file[typeImg];
        if (picInfo) {
            if (picInfo.size <= 0) {
                // 删除空文件
