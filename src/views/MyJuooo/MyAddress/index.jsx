            ...
            <div>
                                ...
                                this.state.addressList.map(v => (
                                    <div key={v._id} className={myAddressCss.addressItem}>
                                        <div className={myAddressCss.addressInfo}>
                                            <p>{v.consignee_name} <span>{v.consignee_number}</span> {v.is_default ? <b>默认</b> : ''}  </p>
                                            <h5><span>{v.disdtict}</span>{v.street}</h5>
                                        </div>
                                        <input type="button" className={myAddressCss.editBtn} value="编辑" />
                                    </div>
                                ))
                            }
                        </div>
                ...
                {this.state.addFromShow ?
                    <div className={myAddressCss.myAddressInfo} >
                        ...
                        <p className={myAddressCss.infoItem} style={{ borderTop: '1px solid #c5c5c5' }}>
                            <span>详细地址:</span>
                            <input name='street'
                                placeholder='请输入详细地址'
                        ...
                        <List.Item
                            ...
                            />}
                        ><p>设为默认地址</p></List.Item>
                    </div> : ''
                }
                <ButtonFoot
                    buttonValue={this.state.addFromShow ? '保存' : '+添加收货地址'}
                    buttonFootFn={this.state.addFromShow ? this.addShippingAddress.bind(this) : this.changeaddFromShow.bind(this)}
                ></ButtonFoot>
            </div >
        );
    }
