...
export default {
  ...
  gethotRecommendList() {
    ...
    return async (dispatch) => {
      ...
      dispatch(
        getClassifyList(homeActionType.GET_CLASSIFY_HOME, data.classify_list)
      ); //首页类型
    };
  },
  getVipHomeSchedular() {
    return async (dispatch) => {
      ...
      // console.log(data);
      dispatch(
        getClassifyList(homeActionType.GET_TOUR_LIST, data.discountList)
      );
    };
  },
  //热门演出
  getHotsRecommendList() {
    return async (dispatch) => {
      ...
      dispatch(
        getClassifyList(
          homeActionType.GET_HOTS_RECOMMENDLIST,
          data.hots_show_list
        )
      );
    };
  ...
  getTourList() {
    return async (dispatch) => {
      const data = await axios.get("show/tour/getList?version=6.1.1&referer=2");
      // console.log(data);
